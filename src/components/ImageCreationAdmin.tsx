...
function CoverPreview({ src, alt, fit = 'cover', className = '' }: { src: string, alt: string, fit?: ImageCreationTemplateDocument['cover_fit'], className?: string }) {
  ...
  return (
    <div className={`relative overflow-hidden bg-gray-100 dark:bg-gray-900 ${className}`}>
      {fit === 'contain' && <><img src={src} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full scale-110 object-cover opacity-35 blur-xl dark:opacity-25" /><span className="absolute inset-0 bg-black/10 dark:bg-black/35" /></>}
      <img src={src} alt={alt} className={`relative h-full w-full ${fit === 'contain' ? 'object-contain p-2' : 'object-cover'}`} />
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/35 p-4 backdrop-blur-sm" onClick={onClose}>
      <article className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl dark:bg-gray-900" onClick={(event) => event.stopPropagation()}>
        ...
        <CoverPreview src={coverUrl} alt={document.cover_alt} fit={document.cover_fit} className="aspect-[4/5] rounded-2xl" />
