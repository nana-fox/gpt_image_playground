...
本轮检查了登录、创作、灵感、作品、额度、账户设置和运营管理。桌面视觉体系整体稳定，主要问题集中在极窄屏弹层、长身份文本、移动安全区、双滚动条和额度卡引用未定义样式变量；已按共享规则修复，没有另起一套页面风格。
...
6. 移动底栏和正文预留系统安全区。
7. 账户弹层桌面宽度调整为 280 px，720 px 及以下最大 320 px；长邮箱在 721、768、1024 和 1440 px 均保持单行。
8. 账户与额度弹层支持点击外部和 `Esc` 关闭；用户端和运营端 Modal 支持 `Esc`，并锁定根页面滚动。
9. 移除页面 320 px 最小宽度和运营侧栏 `100vw`，修复 320 px 设备出现纵向滚动条时的底部横向滚动条。
...
创作、灵感、作品、额度、设置和运营六个路由已在 320、390、720、721、768、1024 和 1440 px 检查；账户弹层无裁切或文本折行，运营套餐弹窗在 320 和 720 px 内完整收缩。浏览器控制台无新增 warning/error。
