# NanaFox Studio 支付实施记录（历史）

> 当前支付架构、配置边界和验收状态已整理到 `docs/nanafox-studio/payment.md`。本文保留最初实施切片和风险登记，不再作为当前状态入口。
...
| 类别 | 变更 | 回滚动作 | 顺序 |
|-----|-----|--------|------|
| 配置 | 运营端 `accepting_orders=true` | 先在运营端停止接单；保持服务端凭证加载，继续处理迟到回调 | 1 |
| 代码 | billing 路由与 UI | `git revert` 对应提交并重建镜像 | 2 |
| 数据 | 新增套餐、订单表 | 保留订单审计数据，不做破坏性降级 | 3 |
| 告警 | 支付失败日志 | 回滚后继续观察迟到回调至少 24 小时 | 4 |

回滚后可接受状态：用户仍可登录、使用免费/已有额度和查看作品；不能创建新订单；历史订单仍可查单和接收微信回调。只有凭证本身故障时，已支付未履约订单才需要运营核对后使用现有管理员加额补偿。
...
运营渠道：closed -> open 仅允许凭证已就绪时发生；open -> closed 立即停止新建订单。
渠道 closed 不改变已有订单状态，也不阻止验签回调或查单履约。
并发点：重复创建、微信重试回调、用户轮询与回调同时到达。
防护：渠道配置版本号防止运营覆盖；创建幂等键唯一约束；订单行 `FOR UPDATE`；履约和订单状态在一个 PostgreSQL 事务；额度 reference 唯一约束。
```
...
| feature flag | 是否灰度 | `STUDIO_PAYMENT_ENABLED=false` 默认不加载凭证；即使凭证已加载，数据库 `accepting_orders=false` 仍默认停止新下单 |
