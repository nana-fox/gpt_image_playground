...
| 管理员套餐、微信 Native 支付、订单和履约 | `c05054c` 已部署测试环境；运营端接单开关、版本冲突、审计、关闭后历史订单履约均通过真实 PostgreSQL/R2 验证；测试商户配置与真实小额支付验收待完成 |
| PostgreSQL/R2 测试环境部署 | `c05054c` 已部署；`49dc3ba` 停止容器保留为当前测试回滚点 |
