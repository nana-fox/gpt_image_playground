...
- 已实现：灵感内容由 PostgreSQL 保存，运营端可新增、编辑、上下架、推荐和排序；用户收藏暂不跨设备同步。
- 待实现：任务排障、订单列表与退款、审计日志查看。
...
- 失败任务临时对象：最多保留 24 小时。
- 用户删除：立即从正常作品库隐藏并进入“最近删除”；7 天内可恢复，到期由 Studio 清理 R2 对象并保留任务墓碑。不用 Bucket 全局规则误删正常作品。
...
- PostgreSQL 目标：每 6 小时一次逻辑备份，保留 7 个每日、4 个每周、6 个每月并复制到 NAS。测试环境当前已纳入每日共享备份任务，NAS Bucket 实际约 31 天自动过期，尚未达到目标保留期。
...
- RPO 目标：数据库不超过 6 小时；当前每日任务的实际数据库 RPO 仍接近 24 小时。作品在独立 R2 备份上线前仅依赖 R2 自身耐久性。
...
| 管理员套餐、微信 Native 支付、订单和履约 | 已部署测试环境；运营端接单开关、版本冲突、审计、关闭后历史订单履约均通过真实 PostgreSQL/R2 验证；测试商户配置与真实小额支付验收待完成 |
...
| 前端失败闭环、单用户生成并发和超时恢复 | `72820ea`、`de4d160` 已实现并通过本地/临时 PostgreSQL 门禁，待部署测试环境 |
...
| PostgreSQL/R2 测试环境部署 | `0e19078` 已部署；`8b025ff` 停止容器保留为当前测试回滚点 |
