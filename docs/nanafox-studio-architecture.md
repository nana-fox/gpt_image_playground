...
| 能力 | 状态 |
|-----|-----|
| Studio 前端闭环、独立 Session、可配置每日免费次数、管理员加额、真实生成 | 已实现并在现有测试环境验证过；测试部署仍是旧 SQLite/本地文件版本 |
| PostgreSQL Store、migration、并发额度保护 | 代码已完成，尚未切换测试服务器 |
| R2 私有 Store、条件写入、后端代理读取 | 代码已完成并通过单元测试，尚未创建 Bucket/Token、尚未做真实 R2 集成测试 |
| PostgreSQL/R2 测试环境部署 | 待执行 |
