...
- 已实现：微信 Native 与支付宝电脑网站支付供应商由 Studio 运营端独立配置；敏感配置加密保存，页面和审计只返回掩码状态，未配置时支付失败关闭。
- 已实现：生图 Key、Base URL 和 R2 凭证仍只存在服务端 Secret；运营端只显示脱敏状态，并可通过版本化、带审计的 Studio 开关暂停新任务。暂停发生在任务写入和额度冻结之前，不影响进行中任务与历史作品。
