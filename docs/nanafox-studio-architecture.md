...
| 能力 | 状态 |
|-----|-----|
| Studio 前端创作闭环、独立 Session、每日免费额度、真实生成 | 已实现；当前公网测试部署仍是旧 SQLite/本地文件版本 |
| PostgreSQL Store、migration、并发额度保护 | 代码与测试 database/role 已准备，尚未切换公网测试容器 |
| R2 私有 Store、条件写入、后端代理读取 | 代码、私有测试 Bucket 和单桶 Token 已准备，真实 R2 合约测试待执行 |
| 管理员套餐/策略/单用户加额 | Store 方法已存在；受保护管理 API 与真实前端闭环尚未实现，现有管理页不得视为完成 |
| PostgreSQL/R2 测试环境部署 | 阻断修复完成，待真实集成测试、暗部署与切换 |
