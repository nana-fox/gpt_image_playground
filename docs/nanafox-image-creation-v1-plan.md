...
| Sub2API | 测试与生产 `6966a8f5c` | `feature/image-creation-v1` 独立 worktree | V1.0.1 的精选容量、生图 Key 资格和 iframe/新窗口错误分离已通过完整门禁并发布生产；功能分支已推送到 NanaFox `origin`，未推送 `upstream` |
...
- 精选架展示已发布且配置了 `home_position` 的模板，顺序由 `home_position` 决定；第 1 个只增加“本周精选”标识，不再占用独立的大卡槽位。V1.0.1 已在迁移、服务和前端三层统一为最多 20 个。
...
- 生产最多配置 20 个已发布模板；迁移 `239_expand_image_creation_featured_capacity.sql`、服务校验和前端交互保持同一上限。
...
生产发布、独立表迁移、模板同步、回滚点保存和核心路由验收已完成，详细证据见 8.13。后续发布继续要求测试先行、不可变制品、生产备份和显式回滚点。
...
| Sub2API | 容器 `sub2api-prod` 当前使用 `sub2api:prod-image-creation-v1-9d8ab6d7c`，healthy，重启计数 0 | `sub2api-prod-rollback-20260825T042018Z`（镜像 `sub2api:prod-image-creation-v1-ecba95ba3`） |
...
- Playground 初始生产候选提交为 `179c8ebe46c6ba2a1085ccfae6ff257e928ce4e7`；Sub2API 初始生产候选为 `ecba95ba38737da703f06913dbc82d372a9172f5`，随后以 `9d8ab6d7c` 修复页面启动门禁和可用生图 Key 筛选。两个仓库的候选分支均归档到各自 `origin`，没有推送上游仓库。
...
- 2026-08-25 V1.0.1 发布后，生产和测试容器均运行 `6966a8f5c`、healthy、重启计数 0；生产静态指针保持 `179c8eb`，测试静态指针保持 `70fea87`。
...
### 8.15 V1.0.1 生产发布（2026-08-25）

产品方在低使用量时段确认发布。生产只切换 Sub2API 不可变镜像，没有修改 Playground 静态指针、Caddy、生产配置或数据库连接池，也没有推送 `upstream`。

| 项目 | 生产结果 | 回滚点 |
|---|---|---|
| Sub2API | `sub2api-prod` 使用 `sub2api:prod-image-creation-v1-6966a8f5c`，二进制为 `0.1.179 / 6966a8f5c`，healthy，重启计数 0 | 停止保留的 `sub2api-prod-rollback-9d8ab6d7c-20260825T105400Z` |
| 生产数据库 | 迁移 `239_expand_image_creation_featured_capacity.sql` 已应用；`home_position` 约束为 1–20 | `/srv/nanafox/image-playground/backups/prod-before-6966a8f5c-20260825T104831Z.sql.gz`，SHA-256 `1f3ca5801bb846d3d4daa6f86da15a5bb53c4184a5c26845a3156ed065a87c42` |
| 数据完整性 | 101 个模板、101 个素材、2 条用户模板状态、5 个首页精选保持不变 | 同一前置数据库快照及容器、配置元数据快照 |
| 静态隔离 | 生产 `prod-current` 仍为 `releases/179c8eb`；测试 `current` 仍为 `releases/70fea87` | 本轮未切换静态指针 |

发布验证证据：

- 生产 `/health`、根路由、登录、仪表盘、API Key、管理员页面和 `/tools/image-playground/` 均返回 200；旧 `/tools/image-studio/` 继续返回 410。
- 未认证访问用户模板、用户启动票据、管理员模板、管理员启动票据和首页精选接口均返回 401。
- 迁移 239 的 checksum 为 `a3f38233cd40d7b548c26dbf21986ec1b7ccbb7253cb14eafcf758336b340630`；模板和素材计数与发布前一致。
- 新容器启动和发布后日志未发现 panic、fatal、连接耗尽、迁移失败、支付/订单错误或 HTTP 5xx；测试容器和静态环境未变化。
- 产品方已在测试环境完成人工生成验收并确认可发布；生产发布不自动重复生成，避免额外计费和双发。
- Chrome 可以建立控制连接，但生产嵌入页导航连续超时；未切换其他浏览器，也未将自动化工具故障记为视觉通过。本次创建的 Chrome 标签页已关闭。

### 8.16 生成结果参数展示补丁测试部署（2026-08-26）

本轮只更新隔离测试环境的 Playground 静态制品，没有修改或重启 Sub2API，没有变更测试或生产数据库、Caddy 和生产静态指针。

| 项目 | 测试结果 | 回滚点 |
|---|---|---|
| Playground | `current` → `releases/8bba7e8`；63 个文件、1,983,345 bytes，内容树 SHA-256 `68ab1bf98ea66936344f385d775d97b2d28f87929255ad18331137131943a76e` | `releases/70fea87` |
| Sub2API | `sub2api-test` 继续使用 `sub2api:test-image-creation-v1-6966a8f5c`，healthy，重启计数 0 | 本轮未变更 |
| 生产隔离 | `sub2api-prod` 继续使用 `sub2api:prod-image-creation-v1-6966a8f5c`，healthy，重启计数 0；`prod-current` 仍为 `releases/179c8eb` | 无生产变更 |

实现与验证证据：

- 最近创作卡片只紧凑展示最终结果值；请求值与结果值的对照保留在提示与详情中，不再用“参数不一致”提示打断成功流程。
- 最终图片解码宽高优先于冲突的接口尺寸元数据，避免把服务返回声明误当成真实像素尺寸；详细记录仍保留原请求参数。
- TDD 提交为 `582787e`、`161f61e`、`8bba7e8`。定向测试 155 项、普通构建、embedded 构建、43 个测试文件 / 569 项全量测试和 `git diff --check` 均通过。
...
| 首页精选容量声明漂移 | 已关闭：生产迁移 239、服务校验和前端交互统一为最多 20 个 | NanaFox backend/frontend | V1.0.1 featured capacity migration |
| 生图 Key 资格与实际网关规则尚未完全共用 | 已关闭：服务层统一判定已发布生产；后续只需继续监测新增分组类型 | NanaFox backend | V1.0.1 canonical image-key eligibility |

## 9. 统一后续路线

### 9.1 阶段 A：发布收口

- 以本文件维护当前生产状态和后续执行顺序；`nanafox-production-release-plan.md` 仅保留为 2026-08-22 静态版历史记录。
- 所有生产提交先归档到 NanaFox 自己的 `origin` 功能分支，不向 `upstream` 推送；合入长期维护分支前单独做上游差异审查。
- 每次发布记录源码提交、镜像或静态制品、数据库变更、测试证据和回滚点，禁止只保留服务器镜像。

### 9.2 阶段 B：V1.0.1 正确性加固（已完成）

1. 用增量迁移把首页精选位置上限统一到 20，并覆盖第 20 个成功、第 21 个拒绝、删除后顺序压缩和并发 ETag 冲突。
2. 把生图 Key 资格收敛为服务层可复用判定，覆盖 Key 状态、有效期、额度、分组生图权限、分组可用性和用户分组访问权；界面列表与真实网关不得产生两套语义。
3. 分离 iframe 加载失败与新窗口失败的错误和重试动作，补齐 i18n，并以挂载组件测试验证加载、空 Key、弹窗拦截和恢复路径。
4. 已完成测试先行和生产发布；产品方人工生成通过。自动化 Chrome 嵌入页读取仍属于工具链缺口，不替代后续 Safari、移动端和深色模式的持续回归。

### 9.3 阶段 C：内容运营

- 使用现有管理端筛选 101 个模板，优先整理 20–30 个高质量、自有或权利清晰的核心模板。
- 统一标题、分类、标签、来源和权利状态，归档重复、低质量或不可复用模板；不在本阶段增加模板版本表、OSS 或复杂审核系统。

### 9.4 阶段 D：轻量数据驱动

- 内容稳定后再评估模板展示、详情、应用、收藏、搜索无结果和生成成败等最小事件。
- 不采集 raw API Key、完整用户提示词或上传参考图；只有数据证明需要时，才规划跨设备历史、用户投稿、公开作品或对象存储迁移。
