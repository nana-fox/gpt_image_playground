...
- 精选架展示已发布且配置了 `home_position` 的模板，顺序由 `home_position` 决定；第 1 个只增加“本周精选”标识，不再占用独立的大卡槽位。目标容量为 20，但 V1.0 生产数据库约束当前只允许位置 1–6；在 V1.0.1 迁移完成前，任何界面和接口都不得宣称或写入超过 6 个。
...
- V1.0 生产当前最多配置 6 个已发布模板，与数据库 `home_position` 约束一致；V1.0.1 将在迁移、服务和前端三层统一为最多 20 个，迁移完成前不得只放宽前端或服务层。
...
4. 完成普通用户、管理员、无 Key、非生图 Key、多生图 Key、Safari、移动端和深色模式验收后，再按测试环境到生产环境顺序发布。
